    ...
    internal sealed class WeatherPart
    {
        ...
        public Task<WeatherStatus> GetWeatherStatusAsync(int pawnLoadId, CancellationToken ct = default)
        {
            ...
            return _scheduler.ScheduleOnMainThreadAsync(() =>
            {
                ...
                string weather = string.Empty; try { weather = mapRef?.weatherManager?.curWeather?.label ?? mapRef?.weatherManager?.curWeather?.defName ?? string.Empty; } catch (global::System.Exception ex) { RimAI.Core.Source.Infrastructure.Diagnostics.ErrorPolicy.Ignore(ex, "General", "empty-catch-fallback"); }
                float temp = 0f; try { temp = pawn.AmbientTemperature; } catch (global::System.Exception ex) { RimAI.Core.Source.Infrastructure.Diagnostics.ErrorPolicy.Ignore(ex, "General", "empty-catch-fallback"); }
                float glow = 0f; try { var pg = mapRef?.glowGrid?.PsychGlowAt(pawn.Position) ?? PsychGlow.Lit; glow = pg == PsychGlow.Dark ? 0f : (pg == PsychGlow.Lit ? 1f : 0.5f); } catch (global::System.Exception ex) { RimAI.Core.Source.Infrastructure.Diagnostics.ErrorPolicy.Ignore(ex, "General", "empty-catch-fallback"); }
